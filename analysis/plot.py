...
def plot_with_data(df: pd.DataFrame, private: bool = False, percentage: float = 0.98):
    ...
    for idx, repo_type in enumerate(['space', 'model', 'dataset']):
        ...
        if len(df_type) == 0:
            ...
            df_sorted = df_type.sort_values('lfs_size', ascending=False)
            df_sorted['cumulative_percentage'] = df_sorted['lfs_size'].cumsum() / df_sorted['lfs_size'].sum()
            df_percentage = df_sorted[df_sorted['cumulative_percentage'] <= percentage]
            others_sum = df_sorted['lfs_size'].sum() - df_percentage['lfs_size'].sum()
            plot_data = pd.concat([df_percentage, pd.DataFrame({'repo_id': ['Others'], 'lfs_size': [others_sum]})])
            axs[idx].pie(plot_data['lfs_size'], labels=plot_data['repo_id'], autopct='%1.1f%%', startangle=90)
